...
- L3. [RESOLVED] PKCE заявлен в комментарии (`oidc.go:17`), но не используется. Для confidential client допустимо; добавить как defense-in-depth. Исправлено: `Login` генерирует PKCE-verifier (cookie `oauth_verifier`) и шлёт S256-challenge в `AuthCodeURL`; `Callback` завершает обмен с `oauth2.VerifierOption`. Тест `TestOIDCLoginSetsNonce` проверяет наличие S256-challenge.
- L4. [ACCEPTED] `internal/cache/memory.go:35-38` - ленивая чистка просрочки: записи без обращения не удаляются, неограниченный рост `items`. Для тестового backend приемлемо. Решение: memory-cache используется только в тестах/локально (`CACHE=memory`); в проде - Redis с TTL. Оставлено как есть.
...
- L7. [ACCEPTED] N+1 в вызывающем коде (не в store): `ListMRs`/`ListEvents` дёргаются по одному `request_id`. Наблюдение. Решение: вызывается только на детальной странице одного заказа (один `request_id` за запрос), не в списках - реального N+1 на горячем пути нет. Оставлено как есть.
- L8. [RESOLVED] `models.go:130-131` - `GitLabProjectID`/`MRIID` тип `int` (Go) против `INT`/int4 (Postgres). Id > 2^31 даст переполнение на стороне БД. Рекомендация: `BIGINT` либо `int32`. Исправлено: миграция `000012_request_mr_bigint.sql` расширяет `request_mrs.gitlab_project_id`/`mr_iid` до `BIGINT` (Go `int` на целевых платформах 64-битный). Проверено на свежей БД: колонки `bigint`.
- L9. [RESOLVED] `cmd/portal/main.go:182`, `internal/status/poller.go:54-66` - при shutdown текущий тик reconcile не дренируется (нет `WaitGroup`); может остаться полузавершённое состояние в GitLab (висящая ветка). Исправлено: `main` запускает поллер с done-каналом и после остановки HTTP ждёт его завершения (дренаж текущего тика) с ограничением 15s grace, после чего выходит.
- L10. [RESOLVED] `internal/provisioning/reconcile.go:34-37` - `autoMerge` мерджит MR без ревью. Комментарий «off in production», но опасная конфигурация. Рекомендация: fail-fast при `autoMerge=true` + реальный GitLab, либо громкий WARN при старте. Исправлено: при `GITLAB_AUTO_MERGE=true` портал пишет громкий Warn на старте (с `gitlab_mode`). Хард-фейл не ставим: стенд легитимно использует auto-merge для демо.
- L11. [RESOLVED] `internal/status/poller.go:54-66` - retry без backoff; `DiscoverApplications` обходит все проекты группы каждые 15s без circuit breaker. При росте каталога - риск самоDoS на GitLab. Рекомендация: экспоненциальный backoff, кэш discovery. Исправлено: поллер ведёт счётчик подряд-сбоев на reconciler и пропускает его до `nextAttempt` с экспоненциальным backoff `interval * 2^(n-1)`, capped 5m (`backoffFor`); успех сбрасывает счётчик. Часы инъектируемы для тестов. Тесты `TestBackoffFor`, `TestPollerBacksOff`.
- L12. [RESOLVED] `internal/provisioning/gitops.go:160-189` - `application.yaml` рендерится `text/template` без YAML-экранирования. `Cluster` (M10) и `ChartVersion` со спецсимволами могут сломать/подделать структуру (YAML-инъекция). Рекомендация: `yaml.Marshal` структуры либо `%q`-квотинг скаляров. Исправлено: каждый standalone-скаляр прогоняется через `yamlScalar` - безопасные значения остаются bare (нет лишнего drift), небезопасные JSON-экранируются (валидный YAML-скаляр). `Path` исключён (встроен в строку, сегменты валидируются `nameRe`). Тест `TestRenderApplicationEscapesScalars`.
- L13. [RESOLVED] `collector/k8s.go:50-98` - `collect` падает целиком при ошибке одного namespace; теряются данные по всем. `internal/provisioning/drift.go:46-94` - `checkDriftOne` молча проглатывает ошибки store. Рекомендация: логировать и продолжать. Исправлено: `collect` логирует сбойный namespace (Warn) и продолжает, отдавая частичный снапшот; `checkDriftOne` логирует проглоченные ошибки (Debug на недоступный репо, Warn на сбой `SetDrift`).
...
- **FE-L1. [RESOLVED]** `href` из серверных данных без клиентской санитизации: `requestDetailParts.tsx:718` (`m.mr_url`), `RequestDetailPage.tsx:387` (`argocd_url`), `StatusPage.tsx:112` (`c.url`). React не блокирует `javascript:`-URL в проде. Источники сейчас доверенные (URL формирует бэкенд из конфига). Рекомендация: хелпер `safeHref`, пропускающий только `http(s):` (в `DocsPage.tsx:229` для markdown такая проверка уже есть - стоит унифицировать). Исправлено: добавлен `lib/href.ts` `safeHref` (пропускает только `http(s)`); применён в `Field` (argocd_url), `MrRow` (mr_url), `StatusPage` (c.url), `AboutPage` (markdown-ссылки). Тест `href.test.ts`.
- **FE-L2. [RESOLVED]** `new RegExp(s.pattern)` из серверной JSON-схемы (`SchemaForm.tsx:53`) - теоретический ReDoS при кривом паттерне. Схемы проходят согласование, риск низкий. Рекомендация: `try/catch` + лимит длины ввода. Исправлено: `matchesPattern` оборачивает `new RegExp(pattern).test(v)` в try/catch и пропускает ввод > 4096 символов (бэкенд-валидация авторитетна; безопасный fallback - «нет клиентской ошибки»).
- **FE-L3. [RESOLVED]** `rel="noreferrer"` без явного `noopener` на `target="_blank"` (`Markdown.tsx:14`, `DocsPage.tsx:230`, `AboutPage.tsx:28`, `StatusPage.tsx:114`, `requestDetailParts.tsx:91,720`). Современные браузеры применяют `noopener` к `_blank` автоматически, и `noreferrer` его подразумевает - уже закрыто. Косметика: писать `rel="noopener noreferrer"`. Исправлено: во всех перечисленных местах `rel="noopener noreferrer"`.

## Корректность

### High

- **FE-H1. [RESOLVED] `navigate()` в теле рендера** - `OrderPage.tsx:174`. Редирект `navigate(...)` вызывается прямо в render (не в `useEffect`), что в React 19/StrictMode даёт предупреждение «Cannot update a component while rendering» и потенциально двойную навигацию. Рекомендация: `return <Navigate to={...} replace />`. Исправлено: редирект на live-заказ возвращается как `<Navigate to=... replace />` вместо вызова `navigate()` в рендере.
- **FE-H2. [RESOLVED] Нет отмены запросов в `useAsync`** - `useAsync.ts:20-38` + `client.ts:59-83`. Флаг `alive` защищает от setState после unmount, но сам fetch не отменяется (нет `AbortController`). При часто меняющихся deps (OrderPage `[project, name, effectiveVersion]`) накапливаются висящие запросы к `/schema`, `/charts`, `/catalog`. Рекомендация: пробрасывать `AbortSignal` из `useAsync` в `fetch`, в cleanup вызывать `abort()`. Исправлено: `useAsync` создаёт `AbortController`, передаёт `signal` в fn и вызывает `abort()` в cleanup (AbortError игнорируется). `req`/GET-методы клиента принимают `signal`; OrderPage (хотспот) пробрасывает его в `getRequest`/`getChart`/`getAggregatedChangelog`/`getSchema`/`getChartView`. Старые вызовы `() => api.x()` остаются совместимы по типу.

### Medium

- **FE-M1. [RESOLVED] `MapField` не ресинхронизируется с внешним `value`** - `SchemaForm.tsx:755-766`. `rows` инициализируется через `useState(init)` только при монтировании. При внешнем изменении `value` (переключение form/raw в OrderPage через `setValues(yaml.load(raw))`, гидратация черновика, upgrade-prefill) компонент показывает устаревшие ключи/значения. Рекомендация: сделать контролируемым (выводить rows из `value`) либо синхронизировать через `useEffect`/`key`. Исправлено: `MapField` ресинхронизирует `rows` из `value` через `useEffect`, когда внешнее значение расходится с тем, что представляют строки (свои правки идут через `push` и не клобберятся).
- **FE-M2. [PARTIAL] `key={i}` (index) на изменяемых списках** - `GenericProductTabs.tsx:288` (ListEditor), `SchemaForm.tsx:600,643,773` (ArrayField/MapField). Списки add/remove/reorder; при удалении из середины React переиспользует DOM/состояние по индексу - раскрытое состояние `Disclosure`, фокус и значение инпутов «съезжают» на соседний элемент. Рекомендация: стабильный ключ по id/name элемента или внутренний uid. Исправлено для `MapField` (строки получили стабильный `id`). `ArrayField` и `ListEditor` (`GenericProductTabs`) - отложено: их элементы выводятся из `value` без локальной идентичности, корректные стабильные ключи требуют управления uid в lockstep со всеми мутациями + ресинком; нужна ручная проверка в SPA.
- **FE-M3. [RESOLVED] `SingleField` seed-guard ломается при ресете формы** - `SchemaForm.tsx:713-723`. `seeded.current=true` навсегда после первого засева; при пересоздании values (смена режима/версии в upgrade, гидратация) повторный засев дефолтов не происходит, поле остаётся пустым. Пограничный баг при upgrade/смене версии. Исправлено: одноразовый ref-guard убран, засев привязан к `item === undefined` - дефолты пересеваются при сбросе/пересоздании формы. У `SingleField` нет кнопки удаления, поэтому редактирование не уводит item в undefined и засев не борется с пользователем.
- **FE-M4. [RESOLVED] `semver.parse` молча трактует невалидную версию как `0.0.0`** - `semver.ts:6-12`. `parseInt("abc")` -> NaN -> 0; `"latest"`/`"main"` парсятся как `0.0.0`. Pre-release (`-rc1`) полностью игнорируется, поэтому `1.2.3-rc1` == `1.2.3`, что даёт нестабильную сортировку при наличии rc. В боевом каталоге Harbor отдаёт semver (риск низкий), но импортированные из Git заказы могут иметь нестандартный `chart_version`. Исправлено: `compareSemver` учитывает pre-release по правилам semver (релиз > его pre-release; идентификаторы numeric/alphanumeric), build-metadata (`+...`) отбрасывается. Нечисловые core-части по-прежнему -> 0 (lightweight, оговорено). Заведены фронт-тесты (`bun test`) + `semver.test.ts`; `bun test` добавлен в lefthook и CI (раньше фронт был без тестов).
